...
export default function ArmDeckPhase3() {
  ...
  const playbackDuration = playingTraj?.frames.length
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', background: '#111', display: 'flex', flexDirection: 'column' }}>
      <PhaseLabel label="Phase 3 — Recording & Playback" />
      {robot && (
        <TopBar
          robotId={robotDef.id}
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', paddingTop: robot ? 40 : 0 }}>
        ...
        <div style={{ width: 210, background: '#1a1a2a', overflowY: 'auto', flexShrink: 0 }}>
          <TransportPanel
...
function PhaseLabel({ label }: { label: string }) {
  return (
    <div style={{
      ...
      background: 'rgba(150,80,0,0.8)', color: '#fff',
